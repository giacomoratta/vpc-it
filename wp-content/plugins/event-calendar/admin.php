  ...
  function event_editor_box()
  {
    ...
    <table width="100%" cellspacing="2" cellpadding="5" class="editform">
     <thead><tr>
      ...
      <th style="text-align:center"><?php echo "Non mostrare l'ora"; //_e('All Day','ec3'); ?></th>
...
//   }
    ...
    foreach($sched_entries as $sid => $vals)
    {
      ...
      elseif($action=='update' && $sid>0 && $sid_ok):
			...
			//$_POST['newtag']['post_tag'];
      	
      		$wpdb->query(
         		"UPDATE $ec3->schedule
         		SET	".$this->implode_assoc(', ',$vals).",
					category_id='|".implode("|",$_POST['post_category'])."|'
      ...
      elseif($action=='create'):
      		...
      		$wpdb->query(
        		"INSERT INTO $ec3->schedule
          		(post_id, ".implode(', ',array_keys($vals)).",category_id)
          		VALUES ($post_ID,".implode(', ',array_values($vals)).", '|".implode("|",$_POST['post_category'])."|')"
        	);
